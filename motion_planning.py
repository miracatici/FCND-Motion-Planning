...
from planning_utils import a_star, heuristic, create_grid, bresenham_prune_path, prune_path
...
class MotionPlanning(Drone):
    ...
    def __init__(self, connection, goal_pos_global):
        ...
        self.check_state = {}
        self.goal_pos_global = goal_pos_global
    ...
    def plan_path(self):
        ...
        pruned_path = bresenham_prune_path(path, grid)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lat_target', type=float, default=37.792995, help="Target Latitude")
    parser.add_argument('--lon_target', type=float, default=-122.397070, help="Target Longitude")
    parser.add_argument('--alt_target', type=float, default=5.0, help="Target Altitude")
    args = parser.parse_args()
    target_pos = np.array([args.lon_target, args.lat_target, args.alt_target], dtype='Float64')
    conn = MavlinkConnection('tcp:{0}:{1}'.format(args.host, args.port), timeout=60)
    drone = MotionPlanning(conn, goal_pos_global = target_pos)
    time.sleep(1)
